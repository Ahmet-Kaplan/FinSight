...
class Click(Tool):
    ...
    async def api_function(self, urls: List[str], task: str) -> List[ToolResult]:
        ...
        result_list: List[ClickResult] = []

        # Share a single browser session across all HTML URLs
        async with AsyncWebCrawler(config=browser_conf) as crawler:
            for url in urls:
                try:
                    if self._is_download_url(url) or self._is_download_hint(url):
                        # Route through download/extraction logic (handles PDF, Excel, CSV, DOCX, etc.)
                        content = await self._fetch_download_content(url)
                    else:
                        # Regular HTML page via Playwright
                        try:
                            result = await crawler.arun(url=url, config=run_conf)
                            content = str(result.markdown)
                        except Exception as e:
                            if "Download is starting" in str(e):
                                # Playwright hit a download trigger — fallback to fetch
                                logger.warning(f"Playwright download trigger for {url}, falling back to fetch")
                                content = await self._fetch_download_content(url)
                            else:
                                raise
